...
class btcDetail extends StatefulWidget {
  ...
  btcDetail({Key key, this.currency_data, this.pair, this.familycoin})
      : super(key: key);

  _btcDetailState createState() =>
      _btcDetailState(currency_data: currency_data);
}
...
Future<void> open_order(String listed, String coin, String pair) async {
  ...
  // if(openOrder[0]['bids'].length==null){
  //   length="0";
  // }else {
  //   length = openOrder[0]['bids'].length.toString();
  //
  // }
//buy
...
//sell
  for (int i = 0; i < openOrder[0]['asks'].length; i++) {
    ...
    //     Cr.format(double.parse(openOrder[0]['asks'][i][0].toString()))
    //         .toString();
    // othersellList[i].value =
    //     Cr.format(double.parse(openOrder[0]['asks'][i][1].toString()))
    //         .toString();
    print(" P R O " +
  ...
  // buys.add(double.parse(openOrder[0]['bids'][i][1]));
  // asks.add(double.parse(openOrder[0]['asks'][i][1]));
  // large_value = buys.reduce(max) > asks.reduce(max) ? buys.reduce(max) : asks.reduce(max);
  // anotherbuYlist[i].percent = Cr.format(double.parse(anotherbuYlist[i].value) / large_value)
  //     .toString();
  // othersellList[i].percent =
  //     Cr.format(double.parse(othersellList[i].value) / large_value)
  //         .toString();
      ...
      if (response.statusCode == 200) {
        if (data['status_code'] == '1') {
          ...
          for (var i = 0; i < typesList.length; i++) {
            ...
            list.forEach((element) {
              for (var j = 0; j < listedTickers.length; j++) {
                ...
                if (element['symbol'].toString().toUpperCase() ==
                  ...
                  // log('${Currency_data[typesList[i]][listedTickers[j]][Currency_data[typesList[i]][listedTickers[j]].indexWhere((element) => element.containsKey('currentprice'))]['currentprice']}');
                  // log((element['name'] + element['price']).toString());
                } else {
                  // print('not found');
                }
              }
            ...
            // log(data['data']['${typesList[i]}'][0]['symbol'].toString() +
            //     i.toString());

// print(data['data'][typesList[i]].length);
  ...
  void updateCandlesFromSnapshot(AsyncSnapshot<Object> snapshot) {
    ...
    // print("checkkline" + datasKline[0][''].toString());
  }
  ...
  Future<void> proceedtoBuy(String method) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (data["status_code"] == '1') {
        ...
        //     pageBuilder: (_, __, ___) => new bottomNavBar(index: 2)));
        // Navigator.of(context).push(PageRouteBuilder(
        //     pageBuilder: (_, __, ___) => new Product_History()));
      } else {
        setState(() {
          order = false;
          order1 = false;
      ...
      children: <Widget>[
              ...
              Visibility(
                ...
                child: Row(
                  children: [
                    ...
                    //       });
                    //      },
                    //     child: Icon(expand==false?Icons.arrow_forward_ios_rounded:Icons.arrow_back_ios_rounded,color: Colors.white38,)),
                  ],
                ),
              ),
              SizedBox(
                    ...
                    onTap: () {
                      if (all == true) {
                        setState(() {
                          ...
                          // print("CURRENCY DATA ? ? ? " + currencycompletedata.toString());
                        });
        ...
        Stack(
          children: [
            ...
            //       backgroundColor: Colors.blueGrey,
            //
            //       child: Icon(Icons.sell_sharp,color: Colors.white,)),
            // ),
          ],
        ),
      ],
