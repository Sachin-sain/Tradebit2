...
getcompletedata(int pagenumber) async {
  ...
  var response = await APIMainClassbinance(APIClasses.OpenData, paramDic, "Get");
...
class _CompleteopenOrdersState extends State<CompleteopenOrders> {
  ...
  getCompleteOrderDetail(String id) async {
    ...
    var response = await APIMainClassbinance(APIClasses.completeOrderDetail, paramDic, "Get");
    var data = json.decode(response.body);
   print(data.toString());
    orderdetails.clear();
    if (response.statusCode == 200) {
    ...
    return status=='true'?Padding(
      ...
      child: Stack(
        children: <Widget>[
          ...
          completedata.length>0? NotificationListener<ScrollNotification>(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (BuildContext ctx, int i) {
                  return InkWell(
                    onTap: (){
                      getCompleteOrderDetail(completedata[i]['id'].toString());
                      List senddata=[];
                      setState(() {
                        senddata.add(orderdetails);
                        print("iN DATA ?ewfdklnhjitrnmj,ersfdg  "+senddata[0][0]["data"][0]["commission"].toString());
                      });
                      Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) =>OrderDetail(orderdetail: senddata[0],)));
                    },
                    child: Padding(
                          ...
                          Row(
                            ...
                            children: <Widget>[
                              Container(
                                ...
                                child:
                                Text(
                                  completedata[i]['currency']+"/"+completedata[i]['with_currency'],
                              ...
                              Container(
                                width: MediaQuery.of(context).size.width*0.2,
                                child:
                                Text(completedata[i]['at_price']==null?"": Cr.format(double.parse(completedata[i]['at_price'])),
                                  style: TextStyle(fontFamily: "IBM Plex Sans", fontSize: 12.0,color: day==false?Colors.white:Colors.black),textAlign: TextAlign.center,
                                ),
                                // Container(
                                // width: 50,
                                // child: Divider(color: day==false?Colors.white38:Colors.black38)),
                              ...
                              Container(
                                ...
                                width: MediaQuery.of(context).size.width*0.2,
                                child: Text(
                                  completedata[i]['quantity']==null?"":completedata[i]['quantity'],
                                  style: TextStyle(fontFamily: "IBM Plex Sans", fontSize: 12.0,color:day==false?Colors.white:Colors.black),textAlign: TextAlign.center,
                                ),
                              ),
                              Container(
                                ...
                                child: Column(
                                  children: [
                                    Text(
                                      completedata[i]['total']==null?"":completedata[i]['total'],
                                      style: TextStyle(fontFamily: "IBM Plex Sans", fontSize: 12.0,color: day==false?Colors.white:Colors.black),textAlign: TextAlign.center,
                                    ),
                                    Container(
                                      //color: Colors.red,
                                      height: 25,
                                      width:80,
                                      child: Center(child: Text(completedata[i]['current_status']==null?"":completedata[i]['current_status'],style: TextStyle(fontFamily: "IBM Plex Sans", fontSize: 8.0,color: completedata[i]['current_status']=='canceled'?Colors.red:completedata[i]['current_status']=='buy'?Colors.redAccent:Colors.greenAccent),textAlign: TextAlign.center,)),
                                    ),
                                  ],
                                ),
                              ),
          ...
          Center(child: Container(
            child: Text("No Complete Orders! \n Let's Order Some Crypto !",style: TextStyle(color: day==false?Colors.white:Colors.black,fontFamily: 'IBM Plex Sans'),),)),

          Align(
            alignment: Alignment.bottomCenter,
            child:
            _isTrackProgress?Text(""): Container(height: 20,width:20,child: CircularProgressIndicator(backgroundColor: Colors.white,strokeWidth: 5,)),
