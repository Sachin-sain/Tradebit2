  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      routes: <String, WidgetBuilder>{
        "onBoarding": (BuildContext context) => firstTime == "false"
            ? onBoarding()
            : bottomNavBar(
                index: 0,
              ),
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  startTime() async {
    return new Timer(Duration(milliseconds: 8500), NavigatorPage);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color(0xff364f66),
      body: Container(
        height: MediaQuery.of(context).size.height,
        child: Image.asset("assets/ilustration/splash.gif",
            height: MediaQuery.of(context).size.height),
      ),
    );
  }
